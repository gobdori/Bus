...
import bus.db.Progress;
import bus.db.ProgressManagerDao;
import bus.db.ProgressTableModel;

public class RsvInfoPnl extends JPanel {

	Vector<Progress> progressList;
	ProgressManagerDao dao;
	JTable progressTable;
	ProgressTableModel progressModel;
	JButton btnSign;
	...
	RsvInfoPnl() {
		...
		dao = new ProgressManagerDao();
		progressList = dao.getProgressList();
		 ...
		 */
		progressModel = new ProgressTableModel(progressList);
		...
		/* 테이블의 크기를 지정하고 있다. */
		progressTable
				.setPreferredScrollableViewportSize(new Dimension(610, 30));
		...
		centerPanel.setPreferredSize(new Dimension(618, 250));
		...
		southPanel.setPreferredSize(new Dimension(585, 150));
